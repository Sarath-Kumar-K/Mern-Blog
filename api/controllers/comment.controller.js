...
export const editComment = async (req, res, next) => {
  try {
    console.log("the api controller called");
    const comment = await Comment.findById(req.params.commentId);
    if (!comment) {
      console.log("comment not found in the server");
      return next(errorHandler(404, "Comment not found"));
    }
...
export const deleteComment = async (req, res, next) => {
  ...
  try {
    ...
    if (comment.userId !== req.user.id || !req.user.isAdmin) {
      ...
      return next(
        errorHandler(403, "You are not authorized to delete the comment")
      );
    }
...
export const getComments = async (req, res, next) => {
  if (!req.user.isAdmin) {
    return next(errorHandler(403, "You are not authorized to get comments"));
  }
